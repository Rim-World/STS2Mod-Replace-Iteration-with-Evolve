...
$modOut = Join-Path $game 'mods\EvolveReplacementMod'

Write-Host "==> Build .NET assembly"
dotnet build (Join-Path $root 'EvolveReplacementMod.csproj') -c Debug
if ($LASTEXITCODE -ne 0) { throw 'dotnet build failed' }

$dll = Join-Path $root '.godot\mono\temp\bin\Debug\EvolveReplacementMod.dll'
$pdb = Join-Path $root '.godot\mono\temp\bin\Debug\EvolveReplacementMod.pdb'
...
try {
    ...
    $pck = Join-Path $modOut 'EvolveReplacementMod.pck'
...
Copy-Item $dll (Join-Path $modOut 'EvolveReplacementMod.dll') -Force
if (Test-Path $pdb) { Copy-Item $pdb (Join-Path $modOut 'EvolveReplacementMod.pdb') -Force }
Copy-Item (Join-Path $root 'EvolveReplacementMod.json') (Join-Path $modOut 'EvolveReplacementMod.json') -Force
